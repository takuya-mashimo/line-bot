class LineBotController < ApplicationController
    ...
    def callback
      ...
      singnature = request.env['HTTP_X_LINE_SIGNATURE']
      unless client.validate_signature(body, singnature)
      ...
      events.each { |event|
        ...
        when Line::Bot::Event::Message
          ...
          when LIne::Bot::Event::MessageType::Text
            ...
            #　当日朝のメッセージの送信の下限値は20%としているが、明日・明後日雨がフル稼働かの下限値は30%とする
            min_per = 30
            case input
              # 「明日」or「あした」というワードが含まれている場合
            when /.*(明日|あした).*/
              # info[2]:明日の天気
              per06to12 = doc.elements[xpath + 'info[2]/rainfallchance/period[2]'].text
              per12to18 = doc.elements[xpath + 'info[3]/rainfallchance/period[3]'].text
              per18to24 = doc.elements[xpath + 'info[4]/rainfallchance/period[4]'].text
              if per06to12.to_i >= min_per || per12to18.to_i >= min_per || per18to24.to_i >= min_per
                push =
                 "明日の天気だよね。\n明日は雨が降りそうだよ(>_<)\n今のところ降水確率はこんな感じだよ。\n    6~12時 #{per06to12}%\n 12~18時 #{per12to18}%\n 18~24時 #{per18to24}%\nまた明日の朝の最新天気予報で雨が降りそうだったら教えるね！"
              else
                push =
